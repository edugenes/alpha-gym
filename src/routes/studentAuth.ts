...
const studentSignOpts = { expiresIn: JWT_EXPIRES_IN } as SignOptions;

function normalizeCpfOrEmail(input: string): string {
...
studentAuthRouter.post("/login", async (req, res) => {
  try {
    const { cpfOrEmail, password } = req.body as { cpfOrEmail?: string; password?: string };
    ...
    const row = await db.query(
      isEmail
        ...
        : "SELECT id, name, cpf, email, status, plan_name, due_date, password_hash FROM students WHERE REPLACE(REPLACE(REPLACE(cpf, '.', ''), '-', ''), ' ', '') = $1",
      [normalized]
    );
    ...
    const student = row.rows[0] as { id: number; name: string; cpf: string; email: string | null; status: string; plan_name: string | null; due_date: string | null; password_hash: string | null };
...
studentAuthRouter.get("/me", requireStudentAuth, async (req, res) => {
  try {
    ...
    const student = await db.query(
      `SELECT id, name, cpf, email, status, plan_name, due_date FROM students WHERE id = $1`,
      [studentId]
    );
    ...
    const s = student.rows[0] as { id: number; name: string; cpf: string; email: string | null; status: string; plan_name: string | null; due_date: string | null };
    const dueDate = s.due_date;

    let nextPayment: { dueDate: string; amount: number } | null = null;
    const nextInst = await db.query(
      `SELECT due_date, amount FROM installments WHERE student_id = $1 AND status = 'pending' AND date(due_date) >= date('now') ORDER BY due_date LIMIT 1`,
      [studentId]
    );
    if (nextInst.rows.length > 0) {
      const r = nextInst.rows[0] as { due_date: string; amount: string };
      nextPayment = { dueDate: r.due_date, amount: Number(r.amount) };
    }

    const freq = await db.query(
      `SELECT COUNT(*) as count FROM check_ins WHERE student_id = $1 AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')`,
      [studentId]
    );
    const frequencyThisMonth = Number((freq.rows[0] as { count: string }).count);
...
studentAuthRouter.post("/forgot-password", async (req, res) => {
  const { cpfOrEmail } = req.body as { cpfOrEmail?: string };
  if (!cpfOrEmail?.trim()) {
