...
workoutsRouter.get("/templates", async (_req, res) => {
  try {
    ...
    const templates = (result.rows as TemplateRow[]).map((t) => ({
      id: t.id,
      name: t.name,
...
workoutsRouter.get("/templates/:id", async (req, res) => {
  try {
    ...
    const t = template.rows[0] as TemplateRow;
    const items = await db.query(
      `SELECT i.id, i.template_id, i.exercise_id, i.sets, i.reps, i.sort_order, e.name as exercise_name, e.muscle_group
...
workoutsRouter.post("/templates", async (req, res) => {
  try {
    ...
    if (!body.name?.trim()) return res.status(400).json({ error: "Nome da ficha é obrigatório." });
    await ensureWorkoutTemplatesTable();
    const db = getDb();
    const insert = await db.query("INSERT INTO workout_templates (name) VALUES ($1) RETURNING *", [body.name.trim()]);
    const t = insert.rows[0] as TemplateRow;
...
workoutsRouter.put("/templates/:id", async (req, res) => {
  try {
    ...
    const t = result.rows[0] as TemplateRow;
...
workoutsRouter.post("/templates/:id/items", async (req, res) => {
  try {
    ...
    const body = req.body as { exerciseId: number; sets?: string; reps?: string };
    if (!body.exerciseId) return res.status(400).json({ error: "exerciseId é obrigatório." });
    await ensureWorkoutTemplateItemsTable();
    await ensureExercisesTable();
    const db = getDb();
    const maxOrder = await db.query(
      "SELECT COALESCE(MAX(sort_order), 0) + 1 as next_order FROM workout_template_items WHERE template_id = $1",
      [templateId]
    );
    const nextOrder = (maxOrder.rows[0] as { next_order: number }).next_order;
    const insert = await db.query(
      "INSERT INTO workout_template_items (template_id, exercise_id, sets, reps, sort_order) VALUES ($1, $2, $3, $4, $5) RETURNING id, template_id, exercise_id, sets, reps, sort_order",
      [templateId, body.exerciseId, body.sets ?? null, body.reps ?? null, nextOrder]
    );
    const i = insert.rows[0] as ItemRow;
    const ex = await db.query("SELECT name, muscle_group FROM exercises WHERE id = $1", [body.exerciseId]);
    const exRow = ex.rows[0] as { name: string; muscle_group: string };
    res.status(201).json({
      item: {
        id: i.id,
        exerciseId: i.exercise_id,
...
workoutsRouter.delete("/templates/:templateId/items/:itemId", async (req, res) => {
  try {
    ...
    const result = await db.query(
      "DELETE FROM workout_template_items WHERE id = $1 AND template_id = $2 RETURNING id",
      [itemId, templateId]
    );
...
workoutsRouter.get("/students/:studentId", async (req, res) => {
  try {
    ...
    const list = (result.rows as { id: number; student_id: number; template_id: number; assigned_at: string; template_name: string }[]).map(
      (r) => ({
        id: r.id,
...
workoutsRouter.post("/students/:studentId/templates/:templateId", async (req, res) => {
  try {
    ...
    if (template.rows.length === 0) return res.status(404).json({ error: "Ficha não encontrada." });
    const insert = await db.query(
      "INSERT INTO student_workouts (student_id, template_id) VALUES ($1, $2) RETURNING id, student_id, template_id, assigned_at",
      [studentId, templateId]
    );
    const r = insert.rows[0] as { id: number; student_id: number; template_id: number; assigned_at: string };
    const t = template.rows[0] as { name: string };
    res.status(201).json({
      workout: {
        id: r.id,
        studentId: r.student_id,
...
workoutsRouter.delete("/students/:studentId/workouts/:workoutId", async (req, res) => {
  try {
    ...
    const db = getDb();
    const result = await db.query(
      "DELETE FROM student_workouts WHERE id = $1 AND student_id = $2 RETURNING id",
      [workoutId, studentId]
    );
