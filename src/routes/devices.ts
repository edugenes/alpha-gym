...
function rowToDevice(r: DeviceRow) {
  return {
    id: r.id,
    name: r.name,
    type: r.type,
...
devicesRouter.get("/", requireAuth, requireRole("administrador"), async (_req, res) => {
  try {
    ...
    res.json({ devices: (result.rows as DeviceRow[]).map(rowToDevice) });
  } catch (e) {
...
devicesRouter.post("/", requireAuth, requireRole("administrador"), async (req, res) => {
  try {
    ...
    const insert = await db.query(
      "INSERT INTO devices (name, type, location, token) VALUES ($1, $2, $3, $4) RETURNING *",
      [name.trim(), type, location?.trim() ?? null, token]
    );
    res.status(201).json({ device: rowToDevice(insert.rows[0] as DeviceRow) });
  } catch (e) {
...
devicesRouter.put("/:id", requireAuth, requireRole("administrador"), async (req, res) => {
  try {
    ...
    const { name, location, status } = req.body as { name?: string; location?: string; status?: string };
    await ensureDevicesTable();
    const db = getDb();
    const result = await db.query(
      ...
      [name?.trim() ?? null, location?.trim() ?? null, status ?? null, id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: "Dispositivo não encontrado." });
    res.json({ device: rowToDevice(result.rows[0] as DeviceRow) });
  } catch (e) {
...
devicesRouter.post("/:id/regenerate-token", requireAuth, requireRole("administrador"), async (req, res) => {
  try {
    ...
    const db = getDb();
    const result = await db.query(
      "UPDATE devices SET token = $1 WHERE id = $2 RETURNING *",
      [newToken, id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: "Dispositivo não encontrado." });
    res.json({ device: rowToDevice(result.rows[0] as DeviceRow) });
