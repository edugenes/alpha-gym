...
export type Role = "administrador" | "recepcionista" | "professor";
...
authRouter.get("/me", requireAuth, async (req, res) => {
  try {
    ...
    const user = row.rows[0] as { id: number; email: string; name: string; role: Role };
...
authRouter.get("/users", requireAuth, requireRole("administrador"), async (req, res) => {
  try {
    ...
    const users = (result.rows as { id: number; email: string; name: string; role: string; created_at: string }[]).map((r) => ({
      id: r.id,
      email: r.email,
      name: r.name,
      role: r.role,
...
authRouter.post("/register", requireAuth, requireRole("administrador"), async (req, res) => {
  try {
    ...
    const allowedRoles: Role[] = ["administrador", "recepcionista", "professor"];
    ...
    const user = insert.rows[0] as { id: number; email: string; name: string; role: Role };
