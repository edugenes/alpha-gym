...
function rowToAssessment(r: AssessmentRow) {
  return {
    id: r.id,
...
assessmentsRouter.get("/", async (req, res) => {
  try {
    ...
    res.json({ assessments: (result.rows as AssessmentRow[]).map((r) => rowToAssessment(r)) });
  } catch (e) {
...
assessmentsRouter.get("/:id", async (req, res) => {
  try {
    ...
    res.json({ assessment: rowToAssessment(result.rows[0] as AssessmentRow) });
  } catch (e) {
...
assessmentsRouter.post("/", async (req, res) => {
  try {
    ...
    const insert = await db.query(
      `INSERT INTO assessments (student_id, assessment_date, weight, height, imc, fat_percent, lean_mass, measures, photo_before_url, photo_after_url, notes)
    ...
    res.status(201).json({ assessment: rowToAssessment(insert.rows[0] as AssessmentRow) });
  } catch (e) {
...
assessmentsRouter.put("/:id", async (req, res) => {
  try {
    ...
    if (existing.rows.length === 0) return res.status(404).json({ error: "Avaliação não encontrada." });
    const row = existing.rows[0] as AssessmentRow;
    const weight = body.weight ?? Number(row.weight);
    const height = body.height ?? Number(row.height);
    const imc = calcImc(weight, height);
    await db.query(
      `UPDATE assessments SET assessment_date = $1, weight = $2, height = $3, imc = $4, fat_percent = $5, lean_mass = $6, measures = $7, photo_before_url = $8, photo_after_url = $9, notes = $10
       WHERE id = $11`,
      [
    ...
    res.json({ assessment: rowToAssessment(updated.rows[0] as AssessmentRow) });
  } catch (e) {
