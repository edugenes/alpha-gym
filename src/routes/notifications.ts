...
notificationsRouter.post("/", async (req, res) => {
  try {
    const body = req.body as { studentId: number; title: string; body?: string };
    ...
    const insert = await db.query(
      "INSERT INTO notifications (student_id, title, body) VALUES ($1, $2, $3) RETURNING id, student_id, title, body, created_at",
      [body.studentId, body.title.trim(), body.body?.trim() ?? null]
    );
    const r = insert.rows[0] as { id: number; student_id: number; title: string; body: string | null; created_at: string };
    res.status(201).json({
      notification: {
        id: r.id,
