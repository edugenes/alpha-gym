...
function rowToEmployee(r: EmployeeRow) {
  return {
    id: r.id,
    name: r.name,
    role: r.role,
...
employeesRouter.get("/", async (_req, res) => {
  try {
    ...
    res.json({ employees: (result.rows as EmployeeRow[]).map((r) => rowToEmployee(r)) });
  } catch (e) {
...
employeesRouter.get("/:id", async (req, res) => {
  try {
    ...
    res.json({ employee: rowToEmployee(result.rows[0] as EmployeeRow) });
  } catch (e) {
...
employeesRouter.post("/", async (req, res) => {
  try {
    const body = req.body as { name: string; role: string; commissionPercent?: number; monthlyGoal?: number; status?: string };
    ...
    const insert = await db.query(
      `INSERT INTO employees (name, role, commission_percent, monthly_goal, status)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [body.name.trim(), body.role.trim(), body.commissionPercent ?? null, body.monthlyGoal ?? null, status]
    );
    res.status(201).json({ employee: rowToEmployee(insert.rows[0] as EmployeeRow) });
  } catch (e) {
...
employeesRouter.put("/:id", async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE employees SET name = COALESCE($1, name), role = COALESCE($2, role), commission_percent = $3, monthly_goal = $4, status = $5, updated_at = datetime('now') WHERE id = $6`,
      [
    ...
    res.json({ employee: rowToEmployee(updated.rows[0] as EmployeeRow) });
  } catch (e) {
