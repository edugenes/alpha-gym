...
async function seedDemo() {
  ...
  await db.query(
    "DELETE FROM users WHERE email IN ('admin', 'admin@alphagym.local', 'recepcao@alphagym.local', 'professor@alphagym.local')"
  );

  const usersToInsert: { email: string; name: string; role: string; password: string }[] = [
  ...
  for (const u of usersToInsert) {
    ...
    await db.query(
      ...
      [u.email.toLowerCase(), hash, u.name, u.role]
    );
  }
  ...
  `);
  const plans = (await db.query("SELECT id, name, price FROM plans ORDER BY id")).rows as { id: number; name: string; price: number }[];
  console.log("Planos OK:", plans.length);

  const studentHash = await bcrypt.hash("1234", 10);
  const now = new Date();
  const studentsData: { name: string; cpf: string; email: string; status: "ativo" | "inadimplente" | "cancelado"; planName: string | null; dueDate: Date | null; createdMonthsAgo: number }[] = [
  ...
  for (const s of studentsData) {
    ...
    await db.query(
      `INSERT INTO students (name, cpf, email, status, plan_name, due_date, password_hash, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [s.name, s.cpf, s.email, s.status, s.planName, due, s.status !== "cancelado" ? studentHash : null, createdAt.toISOString()]
    );
  }
  const students = (await db.query("SELECT id, name, plan_name FROM students ORDER BY id")).rows as { id: number; name: string; plan_name: string | null }[];
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    await db.query(
      `INSERT INTO enrollments (student_id, plan_id, start_date, end_date, active) VALUES ($1, $2, $3, $4, 1)`,
      [s.id, planId, dateStr(start), dateStr(end)]
    );
  }
  ...
  for (let m = -3; m <= 1; m++) {
    ...
    for (const s of students) {
      ...
      await db.query(
        `INSERT INTO installments (student_id, plan_id, due_date, amount, status, paid_at) VALUES ($1, $2, $3, $4, $5, $6)`,
        [s.id, planId, dueDate, amount, status, paidAt]
      );
    }
  }
  ...
  for (let i = 0; i < Math.min(6, students.length); i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await db.query(
        `INSERT INTO assessments (student_id, assessment_date, weight, height, imc, fat_percent, lean_mass, notes) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [studentId, assessmentDates[j], weight.toFixed(2), height, imc, fat.toFixed(2), lean, j === 2 ? "Evolução positiva" : null]
      );
    }
  }
  ...
  const exercises = (await db.query("SELECT id, name, muscle_group FROM exercises ORDER BY id")).rows as { id: number; name: string; muscle_group: string }[];
  ...
  const templates = (await db.query("SELECT id, name FROM workout_templates ORDER BY id")).rows as { id: number; name: string }[];
  const ex = (i: number) => exercises[i].id;
  const templateItems: { templateIndex: number; exerciseIndexes: number[]; sets: string[]; reps: string[] }[] = [
  ...
  for (const row of templateItems) {
    ...
    for (let idx = 0; idx < row.exerciseIndexes.length; idx++) {
      ...
      await db.query(
        `INSERT INTO workout_template_items (template_id, exercise_id, sets, reps, sort_order) VALUES ($1, $2, $3, $4, $5)`,
        [templateId, exerciseId, row.sets[idx], row.reps[idx], idx]
      );
    }
  }
  ...
  for (let i = 0; i < Math.min(7, students.length); i++) {
    if (students[i].plan_name === null) continue;
    const templateId = templates[i % 3].id;
    const ins = await db.query(
      `INSERT INTO student_workouts (student_id, template_id) VALUES ($1, $2) RETURNING id`,
      [students[i].id, templateId]
    );
    const swId = (ins.rows[0] as { id: number }).id;
    if (i % 2 === 0) {
  ...
  for (let d = 0; d < 30; d++) {
    ...
    for (let c = 0; c < count; c++) {
      ...
      await db.query(
        `INSERT INTO check_ins (student_id, created_at) VALUES ($1, $2)`,
        [studentId, createdAt.toISOString()]
      );
    }
  }
  ...
  await db.query(
    `INSERT INTO notifications (student_id, title, body, read_at, created_at) VALUES ($1, 'Lembrete de vencimento', 'Sua mensalidade vence em 10 dias. Evite atrasos!', NULL, $2), ($3, 'Novo treino disponível', 'Sua ficha B foi atualizada. Confira no app.', $4, $5), ($6, 'Promoção Anual', 'Renove no plano anual e ganhe 1 mês. Válido até o fim do mês.', NULL, $7)`,
    [students[0].id, twoDaysAgo.toISOString(), students[1].id, oneDayAgo.toISOString(), threeDaysAgo.toISOString(), students[2].id, fiveDaysAgo.toISOString()]
  );
