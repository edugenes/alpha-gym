 ...
 */
export function pollock3Men(chest, abdominal, thigh, age) {
 ...
 */
export function pollock3Women(triceps, suprailiac, thigh, age) {
 ...
 * Equação: Jackson & Pollock, 1978.
 */
export function pollock7Men(chest, midaxillary, triceps, subscapular, abdominal, suprailiac, thigh, age) {
 ...
 * Equação: Jackson, Pollock & Ward, 1980.
 */
export function pollock7Women(chest, midaxillary, triceps, subscapular, abdominal, suprailiac, thigh, age) {
    const sum = chest + midaxillary + triceps + subscapular + abdominal + suprailiac + thigh;
    return 1.097 - 0.00046971 * sum + 0.00000056 * sum * sum - 0.00012828 * age;
}
// ── Equação de Siri ───────────────────────────────────────────────────────────
/** %Gordura = (495 / Densidade Corporal) − 450  (Siri, 1956) */
export function siriBodyFat(density) {
    if (density <= 0)
        return 0;
    return Math.round(((495 / density) - 450) * 100) / 100;
}
export function calcBodyComposition(protocol, sex, age, weight_kg, skinfolds) {
    let density = 0;
    const sf = skinfolds;
    if (protocol === "pollock3") {
        if (sex === "M") {
            if (sf.chest == null || sf.abdominal == null || sf.thigh == null)
                return null;
            density = pollock3Men(sf.chest, sf.abdominal, sf.thigh, age);
        }
        else {
            if (sf.triceps == null || sf.suprailiac == null || sf.thigh == null)
                return null;
