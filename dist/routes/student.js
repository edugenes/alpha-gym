...
import { ensureStudentsTable, ensureStudentWorkoutsTable, ensureWorkoutTemplatesTable, ensureWorkoutTemplateItemsTable, ensureExercisesTable, ensureAssessmentsTable, ensureInstallmentsTable, ensureCheckInsTable, ensureWorkoutCompletionsTable, ensureNotificationsTable, } from "../db/schema.js";
...
studentRouter.get("/workouts/:studentWorkoutId", async (req, res) => {
    try {
        ...
        if (Number.isNaN(studentWorkoutId))
        ...
        const db = getDb();
        const sw = await db.query("SELECT id, template_id FROM student_workouts WHERE id = $1 AND student_id = $2", [studentWorkoutId, studentId]);
        if (sw.rows.length === 0)
            return res.status(404).json({ error: "Ficha não encontrada." });
        const { template_id: templateId } = sw.rows[0];
        const template = await db.query("SELECT id, name FROM workout_templates WHERE id = $1", [templateId]);
        if (template.rows.length === 0)
            return res.status(404).json({ error: "Ficha não encontrada." });
        const items = await db.query(`SELECT i.id, i.exercise_id, i.sets, i.reps, i.sort_order, e.name as exercise_name, e.muscle_group
...
studentRouter.post("/workouts/:studentWorkoutId/complete", async (req, res) => {
    try {
        ...
        await ensureStudentWorkoutsTable();
        const db = getDb();
        const sw = await db.query("SELECT id FROM student_workouts WHERE id = $1 AND student_id = $2", [studentWorkoutId, studentId]);
        if (sw.rows.length === 0)
            return res.status(404).json({ error: "Ficha não encontrada." });
        const insert = await db.query("INSERT INTO workout_completions (student_id, student_workout_id) VALUES ($1, $2) RETURNING id, completed_at", [studentId, studentWorkoutId]);
...
studentRouter.get("/workouts/completions/history", async (req, res) => {
    try {
        ...
        await ensureWorkoutCompletionsTable();
        const db = getDb();
        const result = await db.query(`SELECT wc.id, wc.student_workout_id, wc.completed_at, wt.name as template_name
       ...
       WHERE wc.student_id = $1 ORDER BY wc.completed_at DESC LIMIT 100`, [studentId]);
        const list = result.rows.map((r) => ({
            id: r.id,
...
studentRouter.get("/assessments", async (req, res) => {
    try {
        ...
        const db = getDb();
        const result = await db.query("SELECT id, assessment_date, weight, height, imc, fat_percent, lean_mass, notes, created_at FROM assessments WHERE student_id = $1 ORDER BY assessment_date DESC", [studentId]);
        const list = result.rows.map((r) => ({
            id: r.id,
...
studentRouter.get("/installments", async (req, res) => {
    try {
        ...
        const db = getDb();
        const result = await db.query(`SELECT id, due_date, amount, status, paid_at, created_at FROM installments
       WHERE student_id = $1 ORDER BY due_date DESC`, [studentId]);
        const list = result.rows.map((r) => ({
            id: r.id,
            dueDate: r.due_date,
...
studentRouter.get("/check-ins", async (req, res) => {
    try {
        ...
        const db = getDb();
        const result = await db.query("SELECT id, created_at FROM check_ins WHERE student_id = $1 ORDER BY created_at DESC LIMIT 100", [studentId]);
        const list = result.rows.map((r) => ({
            id: r.id,
            createdAt: r.created_at,
        }));
...
studentRouter.post("/check-in", async (req, res) => {
    try {
        ...
        const status = student.rows[0].status;
        ...
        const insert = await db.query("INSERT INTO check_ins (student_id) VALUES ($1) RETURNING id, student_id, created_at", [studentId]);
        const r = insert.rows[0];
...
studentRouter.get("/notifications", async (req, res) => {
    try {
        ...
        const db = getDb();
        const result = await db.query("SELECT id, title, body, read_at, created_at FROM notifications WHERE student_id = $1 ORDER BY created_at DESC LIMIT 50", [studentId]);
        const list = result.rows.map((r) => ({
            id: r.id,
            title: r.title,
            body: r.body,
...
studentRouter.patch("/notifications/:id/read", async (req, res) => {
    try {
        ...
        await ensureNotificationsTable();
        const db = getDb();
        const result = await db.query("UPDATE notifications SET read_at = COALESCE(read_at, datetime('now')) WHERE id = $1 AND student_id = $2 RETURNING id, read_at", [id, studentId]);
