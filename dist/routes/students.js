...
studentsRouter.get("/", async (req, res) => {
    try {
        ...
        const students = result.rows.map((r) => rowToStudent(r));
        res.json({ students });
    }
    catch (e) {
...
studentsRouter.get("/:id/attachments", async (req, res) => {
    try {
        ...
        const db = getDb();
        const result = await db.query("SELECT id, student_id, type, content_url, file_name, created_at FROM student_attachments WHERE student_id = $1 ORDER BY created_at DESC", [studentId]);
        const attachments = result.rows.map((r) => ({
            id: r.id,
...
studentsRouter.post("/:id/attachments", async (req, res) => {
    try {
        ...
        if (!contentUrl.trim())
            return res.status(400).json({ error: "contentUrl ou content é obrigatório." });
        ...
        const insert = await db.query("INSERT INTO student_attachments (student_id, type, content_url, file_name) VALUES ($1, $2, $3, $4) RETURNING id, student_id, type, content_url, file_name, created_at", [studentId, type, contentUrl.substring(0, 50000), body.fileName?.trim() || null]);
...
studentsRouter.get("/:id", async (req, res) => {
    try {
        ...
        res.json({ student: rowToStudent(result.rows[0]) });
    }
    catch (e) {
...
studentsRouter.post("/", async (req, res) => {
    try {
        ...
        const insert = await db.query(`INSERT INTO students (name, cpf, birth_date, phone, email, address, photo_url, notes, status, plan_name, due_date)
       ...
       RETURNING *`, [
        ...
        res.status(201).json({ student: rowToStudent(insert.rows[0]) });
    }
    catch (e) {
...
studentsRouter.put("/:id", async (req, res) => {
    try {
       ...
       WHERE id = $12`, [
        ...
        res.json({ student: rowToStudent(updated.rows[0]) });
    }
    catch (e) {
