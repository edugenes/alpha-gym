...
financialRouter.get("/installments", async (req, res) => {
    try {
        ...
        const db = getDb();
        await db.query(`UPDATE students SET status = 'inadimplente', updated_at = datetime('now')
       WHERE id IN (SELECT DISTINCT student_id FROM installments WHERE status = 'pending' AND date(due_date) < date('now'))`);
        const { month, year, status, student_id, date_from, date_to } = req.query;
        let query = `
    ...
    `;
        ...
        res.json({
            installments: result.rows.map((r) => formatRow(r)),
        });
    }
    catch (e) {
...
financialRouter.post("/installments/generate", async (req, res) => {
    try {
        ...
        const enrollments = await db.query(`SELECT e.student_id, e.plan_id, p.price
       FROM enrollments e
       JOIN plans p ON p.id = e.plan_id
       WHERE e.active = 1 AND e.start_date <= $1 AND e.end_date >= $2`, [lastDayStr, firstDay]);
        const dueDate = `${year}-${String(month).padStart(2, "0")}-10`;
        let created = 0;
        for (const enr of enrollments.rows) {
            const existing = await db.query("SELECT id FROM installments WHERE student_id = $1 AND due_date = $2", [enr.student_id, dueDate]);
            if (existing.rows.length > 0)
                continue;
            await db.query("INSERT INTO installments (student_id, plan_id, due_date, amount, status) VALUES ($1, $2, $3, $4, 'pending')", [enr.student_id, enr.plan_id, dueDate, enr.price]);
            created++;
        }
...
financialRouter.post("/installments/:id/pay", async (req, res) => {
    try {
        ...
        await ensureInstallmentsTable();
        const db = getDb();
        await db.query("UPDATE installments SET status = 'paid', paid_at = datetime('now') WHERE id = $1", [id]);
        ...
        if (student.rows.length > 0 && student.rows[0].status === "inadimplente") {
...
financialRouter.get("/daily", async (_req, res) => {
    try {
        await ensureInstallmentsTable();
        const db = getDb();
        const result = await db.query(`SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
    ...
    try {
       ...
       WHERE strftime('%m', due_date) = $1 AND strftime('%Y', due_date) = $2`, [mStr, String(y)]);
        ...
        if (format === "csv") {
         ...
         ORDER BY i.due_date, s.name`, [mStr, String(y)]);
