...
function rowToEnrollment(r, planName) {
    return {
        id: r.id,
...
enrollmentsRouter.get("/", async (req, res) => {
    ...
    `;
        ...
        const enrollments = result.rows.map((r) => rowToEnrollment(r, r.plan_name));
...
enrollmentsRouter.get("/student/:studentId/current", async (req, res) => {
    try {
        ...
        await ensurePlansTable();
        const db = getDb();
        const result = await db.query(`SELECT e.*, p.name as plan_name FROM enrollments e
       JOIN plans p ON p.id = e.plan_id
...
enrollmentsRouter.post("/", async (req, res) => {
    try {
        ...
        const insert = await db.query(`INSERT INTO enrollments (student_id, plan_id, start_date, end_date, active)
       VALUES ($1, $2, $3, $4, 1) RETURNING *`, [body.studentId, body.planId, startDate, endDate]);
        await db.query("UPDATE students SET plan_name = $1, due_date = $2, updated_at = datetime('now') WHERE id = $3", [plan.name, endDate, body.studentId]);
