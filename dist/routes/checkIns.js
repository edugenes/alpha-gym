...
import { ensureCheckInsTable, ensureStudentsTable, ensureEnrollmentsTable, ensureInstallmentsTable, ensureStudentWorkoutsTable, ensureWorkoutTemplatesTable, ensureWorkoutTemplateItemsTable, ensureExercisesTable, ensureDevicesTable, } from "../db/schema.js";
...
async function requireDeviceToken(req, res, next) {
    ...
    try {
        ...
        const db = getDb();
        const result = await db.query("SELECT id, name, type FROM devices WHERE token = $1 AND status = 'ativo'", [token]);
...
async function getStudentAccessInfo(studentId) {
    const db = getDb();
    await ensureEnrollmentsTable();
    await ensureInstallmentsTable();
    const enrollment = await db.query(`SELECT e.id, e.end_date, e.active
     FROM enrollments e WHERE e.student_id = $1 AND e.active = 1
     ORDER BY e.end_date DESC LIMIT 1`, [studentId]);
    const hasActiveEnrollment = enrollment.rows.length > 0;
    const overdue = await db.query(`SELECT COUNT(*) as c FROM installments
     WHERE student_id = $1 AND status = 'pending' AND date(due_date) < date('now')`, [studentId]);
    const overdueCount = Number(overdue.rows[0].c);
    return { hasActiveEnrollment, overdueCount };
}
/** Retorna a ficha de treino ativa do aluno (lista de exercícios). */
async function getActiveWorkout(studentId) {
    const db = getDb();
    ...
    const sw = await db.query(`SELECT sw.id, sw.template_id, wt.name as template_name
     FROM student_workouts sw JOIN workout_templates wt ON wt.id = sw.template_id
     WHERE sw.student_id = $1 ORDER BY sw.assigned_at DESC LIMIT 1`, [studentId]);
    if (sw.rows.length === 0)
        return null;
    const r = sw.rows[0];
    const items = await db.query(`SELECT e.name as exercise_name, e.muscle_group, i.sets, i.reps, i.sort_order
     FROM workout_template_items i JOIN exercises e ON e.id = i.exercise_id
     WHERE i.template_id = $1 ORDER BY i.sort_order, i.id`, [r.template_id]);
    return {
        id: r.id,
...
checkInsRouter.post("/", requireAuth, async (req, res) => {
    try {
        ...
        const insert = await db.query("INSERT INTO check_ins (student_id) VALUES ($1) RETURNING id, student_id, created_at", [body.studentId]);
 ...
 */
checkInsRouter.post("/self-service", selfServiceLimiter, requireDeviceToken, async (req, res) => {
    try {
        const { cpf } = req.body;
        if (!cpf?.trim()) {
        ...
        await ensureCheckInsTable();
        const db = getDb();
        const result = await db.query(`SELECT id, name, status FROM students
       WHERE REPLACE(REPLACE(REPLACE(cpf, '.', ''), '-', ''), ' ', '') = $1`, [cpfNormalized]);
        if (result.rows.length === 0) {
        ...
        // Registra check-in
        const insert = await db.query("INSERT INTO check_ins (student_id) VALUES ($1) RETURNING id, created_at", [student.id]);
        const ci = insert.rows[0];
 ...
 */
checkInsRouter.post("/device", requireDeviceToken, async (req, res) => {
    try {
        const { biometricRef, method = "biometria" } = req.body;
        ...
        const result = await db.query("SELECT id, name, status FROM students WHERE biometric_device_ref = $1", [biometricRef.trim()]);
        if (result.rows.length === 0) {
        ...
        const insert = await db.query(`INSERT INTO check_ins (student_id, device_id, method) VALUES ($1, $2, $3)
       RETURNING id, created_at`, [student.id, device.id, method]);
        const ci = insert.rows[0];
        res.json({
            allow: true,
