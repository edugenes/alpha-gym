...
exercisesRouter.get("/", async (req, res) => {
    try {
        ...
        const db = getDb();
        const muscle = req.query.muscle_group;
        ...
        res.json({ exercises: result.rows.map((r) => rowToExercise(r)) });
    }
    catch (e) {
...
exercisesRouter.get("/:id", async (req, res) => {
    try {
        ...
        res.json({ exercise: rowToExercise(result.rows[0]) });
    }
    catch (e) {
...
exercisesRouter.post("/", async (req, res) => {
    try {
        ...
        await ensureExercisesTable();
        const db = getDb();
        const insert = await db.query("INSERT INTO exercises (name, muscle_group, video_url) VALUES ($1, $2, $3) RETURNING *", [body.name.trim(), body.muscleGroup.trim(), body.videoUrl?.trim() || null]);
        res.status(201).json({ exercise: rowToExercise(insert.rows[0]) });
    }
    catch (e) {
...
exercisesRouter.put("/:id", async (req, res) => {
    try {
        ...
        await ensureExercisesTable();
        const db = getDb();
        const result = await db.query("UPDATE exercises SET name = COALESCE($1, name), muscle_group = COALESCE($2, muscle_group), video_url = $3 WHERE id = $4 RETURNING *", [body.name?.trim(), body.muscleGroup?.trim(), body.videoUrl?.trim() ?? null, id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Exercício não encontrado." });
        res.json({ exercise: rowToExercise(result.rows[0]) });
    }
