...
plansRouter.get("/", async (_req, res) => {
    try {
        ...
        res.json({ plans: result.rows.map((r) => rowToPlan(r)) });
    }
    catch (e) {
...
plansRouter.get("/:id", async (req, res) => {
    try {
        ...
        res.json({ plan: rowToPlan(result.rows[0]) });
    }
    catch (e) {
...
const PLAN_TYPES = ["mensal", "trimestral", "semestral", "anual"];
plansRouter.post("/", async (req, res) => {
    try {
        ...
        const planType = (body.planType || "mensal").toLowerCase();
        ...
        const insert = await db.query(`INSERT INTO plans (name, plan_type, price, duration_days, features, is_popular)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [body.name.trim(), planType, body.price, body.durationDays, featuresStr, !!body.isPopular]);
        res.status(201).json({ plan: rowToPlan(insert.rows[0]) });
    }
    catch (e) {
...
plansRouter.put("/:id", async (req, res) => {
    try {
        ...
        const planType = body.planType ? body.planType : row.plan_type;
        const featuresStr = body.features !== undefined ? (Array.isArray(body.features) ? body.features.join("\n") : null) : row.features;
        await db.query(`UPDATE plans SET name = $1, plan_type = $2, price = $3, duration_days = $4, features = $5, is_popular = $6, updated_at = datetime('now') WHERE id = $7`, [body.name ?? row.name, planType, body.price ?? Number(row.price), body.durationDays ?? row.duration_days, featuresStr, body.isPopular ?? row.is_popular, id]);
        const updated = await db.query("SELECT * FROM plans WHERE id = $1", [id]);
        res.json({ plan: rowToPlan(updated.rows[0]) });
    }
    catch (e) {
